...
void MainThread() {
	...
	if (GetModuleHandleA("samp.dll")) {
		...
		details = "Apelido SAMP: " + pSamp->srvData.username;
		...
		while (1 < 2) {
			if (query.info(pSamp->srvData.info)) {
				...
				state = "Nome do servidor: " + pSamp->srvData.info.hostname;
				largeImageText = "Modo de jogo: " + pSamp->srvData.info.gamemode;
				smallImageText = "Número de jogadores: " + players;

			}
			else {
				largeImageText = "Falha ao obter dados do servidor!";
			}
		...
		while (1 < 2) {
			if (pGame->IsPedExists()) {
				if (pGame->IsInCutscene()) {
					details = "Missão: " + pGame->GetCurrentMission();
					state = "Assistindo a Missão";
				}

				else if (pGame->IsInVehicle()) {
					if (pGame->GetVehicleID() >= 400 && pGame->GetVehicleID() <= 611)
						details = "Veículo: " + vehNames[pGame->GetVehicleID() - 400];
					else details = "Hum, não tenho certeza de que veículo é esse!";
					state = "Rádio: " + radioNames[pGame->GetCurrentRadio()];
				}

				else if (pGame->IsAnyMissionActive()) {
					details = "Missão: " + pGame->GetCurrentMission();
					state = "Arma: " + weaponNames[pGame->GetCurrentWeapon()];
				}

				else if (pGame->GetPlayerWantedLevel()) {
					details = "Nível de Procurado: " + std::to_string(pGame->GetPlayerWantedLevel());
					state = "Arma: " + weaponNames[pGame->GetCurrentWeapon()];
				}

				else {
					details = "Dinheiro: R$" + std::to_string(pGame->GetPlayerMoney());
					char helt[64];
					sprintf_s(helt, "Vida: %.2f%%", pGame->GetPlayerHealth());
					state = helt;
				}

				largeImageText = "Localização: " + pGame->GetCurrentZone();
				smallImageText = "Tempo no Jogo: " + pGame->GetTime();
